...
class ChangePINContainer extends Component{
  ...
  async onSave() {
    ...
    const response = await fetch(`${URL_API}Account/changePin`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
            ...
            old_pin: await this.state.old_password,
            new_pin_1: await this.state.new_password_1,
            new_pin_2: await this.state.new_password_2,
            secretkey: 'NOEL'
          })
    });
